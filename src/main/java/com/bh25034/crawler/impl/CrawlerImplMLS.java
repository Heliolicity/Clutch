...
public class CrawlerImplMLS implements Crawler {
	...
	public CrawlerImplMLS() { this.URLs = new ArrayList<String>(); }
	
	public CrawlerImplMLS(String mainURL, String subURL, CSVWriter csvWriter) {
		super();
	...
	public void scrapeLinks() throws IOException {
		...
		try {
			...
			for (Element link : links) {  
				...
				text = link.attr("href");
				index = text.indexOf(",");
				excludeIndex = text.indexOf("gotopage");
				
				if (index > 0 && excludeIndex <= 0) {
				
					this.currentURL = text.substring(index + 2, text.length() - 2).trim() + "/";
					//pl(this.subURL + this.currentURL);
					//pl(text);
					//pl("EXCLUDE: " + excludeIndex);
		    	...
		    	if (element.cssSelector().equals("html")) {
		    		...
		    		if (text.contains("vs")) {
		    			...
		    			try {
		    				...
		    				dateTime = new DateTime(year, month, day, (hour + 1), minute);
			...
			for (Element element : elements) {
		    	...
		    	if (element.cssSelector().contains("tr.score_row")) {
		    		...
		    		if (index > 0) {
		    			...
		    			if (allText.contains("(")) result = allText.substring(index - 2, index + 3).trim();
		    			else result = allText.substring(index - 2, index + 4).trim();
		    			...
		    			if (divider > 0) {
			    			...
			    			if (first.indexOf("'") > 0) {
			    				...
			    				if (first.indexOf("+") > 0) {
			    					...
			    					injGoalMinute = new Integer(first.substring(first.indexOf("+") + 1, first.indexOf("+") + 3).trim()).intValue();
			    					goal.setInjuryTime(true);
			    					...
			    					injGoalMinute = 0;
			    					goal.setInjuryTime(false);
			    					
			    				}
			    				
			    				goal.setTimeScored(goalMinute + injGoalMinute);
			    				space = first.lastIndexOf(" ");
			    		
			    				if ((index + 1) >= (space)) goalScorer = "UNKNOWN";
			    				else goalScorer = first.substring(index + 1, space).trim();
			    		
			    				if (goalScorer.contains("+")) goalScorer = goalScorer.substring(goalScorer.indexOf(" ") + 1, goalScorer.length());
			    				...
			    				if (second.indexOf("+") > 0) {
			    					...
			    					injGoalMinute = new Integer(second.substring(second.indexOf("+") + 1, second.length()).trim()).intValue();
			    					goal.setInjuryTime(true);
			    				...
			    				//goalScorer = third.substring(index + 1, third.length()).trim();
			    				//pl(second);
			    				//pl(third);
		    				...
		    				if (this.isPlayOffs()) goal.setPlayOffGame(true);
		    				else goal.setPlayOffGame(false);
